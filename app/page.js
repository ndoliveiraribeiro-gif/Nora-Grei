...
export default function Home() {
  ...
  const carregarConteudo = async () => {
    const { data: destaque } = await supabase
      .from("pecas").select("nome, fotos, preco_aluguer_dia")
      .eq("destaque", true).not("fotos", "is", null).limit(1).maybeSingle();

    const { data: todasPecas } = await supabase
      .from("pecas").select("nome, fotos, ocasioes, preco_aluguer_dia")
      .not("fotos", "is", null);

    if (destaque?.fotos?.[0]) setHeroPeca(destaque);
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        :root{--black:#080808;--white:#f8f7f5;--g1:#f0eeeb;--g2:#e2dfda;--g4:#5a5855;--rosa:#c4748a;--serif:'Cormorant',Georgia,serif;--sans:'Jost',Arial,sans-serif}
      ...
      <section className="hero">
        <div className="hero-bg"><img src={heroPeca?.fotos?.[0] || FOTO_FALLBACK} alt="" /></div>
        ...
        <div className="hero-right-desktop">
          <img src={heroPeca?.fotos?.[0] || FOTO_FALLBACK} alt={heroPeca?.nome || "Nora Grei"} />
          <div className="hero-caption">
            <span className="hero-caption-nome">{heroPeca?.nome || "Vestido Seda Noite"}</span>
            <span className="hero-caption-preco">{heroPeca?.preco_aluguer_dia ? `${heroPeca.preco_aluguer_dia}€ / dia` : "35€ / dia"}</span>
          </div>
        </div>
      </section>
